...
def getGenesInInterval(interval, genelist, alpha=0):
    ...
    for g_start, g_end in genelist:
        ...
        # partly covered genes
            ...
            cov = getOverlap((g_start, g_end), interval)
...
def getMetric_base(ref_intervals, query_intervals, genelist, options):
        tree = getWindowTree(query_intervals)         
        ...
        '''  
        if options.pttfile:
            tp = len(overlap_total_genes)   
            g_real = len(ref_total_genes)   
        ...
        if options.pttfile:
            format_str = 'Bases Recall: %.3f\tPrecision: %.3f\tF-measure: %.3f\tF-measure Difference: %.3f\tLeft offset: %d\tRight: %d\tPredicted bases: %d\tOverlap bases: %d\tPredicted genes: %d\tOverlap genes: %d\tPredicted intervals: %d\tAverage interval size: %d\tGene Recall: %.3f\tPrecision: %.3f\tF-measure: %.3f\tAvg offset: %d'
            print format_str % (recall, precision, fmeasure, diff_fmeasure, avg_left, avg_right, predicted, overlap_totalSize, g_predicted, len(overlap_total_genes), len(query_intervals), avg_query_len, g_recall, g_precision, g_fmeasure, avg_offset)
        else:
